...
const { div, h1, h2, h5, h6, a, p, span, ul, li, input, label, button } = h
...
function PollTabNavBar ({ poll }) {
  ...
  return ul('.nav.nav-tabs.container.border-0', [
    li('.nav-item', [
      a('.nav-link.text-muted', { href: w.path({ author, pollname }) }, [
    ...
    li('.nav-item', [
      a('.nav-link.active', { href: w.path({ author, pollname, rest: 'settings' }) }, [
        span('.oi.oi-cog.pr-1'),
        'Settings'
      ])
    ])
